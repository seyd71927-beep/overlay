class adminPreStreamInterface {
    ...
    async setActiveTeamSlot(slot, teamTag) {
        ...
        // Populate matchup UI fields
        if (slot === 'team_1') {
            if (this.team1Abbr) this.team1Abbr.value = team.tag || team.name;
            if (this.team1Info) this.team1Info.value = team.seed || '#1 Seed';
            if (this.team1Icon) this.team1Icon.value = team.logo || '';
        } else {
            if (this.team2Abbr) this.team2Abbr.value = team.tag || team.name;
            if (this.team2Info) this.team2Info.value = team.seed || '#2 Seed';
            if (this.team2Icon) this.team2Icon.value = team.logo || '';
        }
    ...
    // ==========================================
    // TEAM CONFIGURATION & MAP VETO LOGIC
    // ==========================================
